...
Cursor's permissions come from the same `rules/rules.json` Claude Code uses. `cursor.nix` turns the allow rules into `~/.cursor/permissions.json` (`terminalAllowlist`, `mcpAllowlist`). Cursor has no deny list, so `~/.cursor/hooks.json` runs jq filters for the deny rules. The `Bash(x:*)` rules ask for approval on `beforeShellExecution`. The `Read(...)` rules deny on `beforeReadFile` and `beforeTabFileRead`. A deny rule in any other parameterised form fails the build. Bare tool names such as `WebFetch` have no Cursor equivalent, so its built-in web tools stay on. `~/.cursor/sandbox.json` takes its network allowlist from `agent-network-domains.nix`, like Claude's sandbox and Codex's network proxy. The run mode (Auto-review, Allowlist, Run Everything) and the sandbox network mode are UI-only in Cursor Settings > Agents.
