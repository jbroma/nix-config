# Public SSH keys of this user's identities; the private halves live in 1Password. Used as git
# signing keys (home-manager/1password.nix, picked by user.email) and as the keys allowed to
# SSH into the LLM server (macos/llm-server.nix). Public material, safe in the repo.
{
  "jakub.romanczyk@callstack.com" =
    "ssh-rsa ";
  "j.romanczyk@gmail.com" =
    "ssh-rsa ";
  "jakub@rork.com" =
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIB+oGPAAbpGPdHylgZdraedv45dHIMG52FgANdWOIyes";
}
